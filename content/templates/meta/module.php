...
<div class="sidebar-user">
	<div class="sidebar-user-img">
		<img src="<?php echo TEMPLATE_URL; ?>inc/api.php?qq=<?php echo $Tconfig["qqhm"]; ?>" alt="晓超云博客">
	</div>
	<div class="sidebar-user-name">
		<a href="mailto:<?php echo $qqhm; ?>@qq.com" title="<?php echo $Tconfig["qqhm"]; ?>@qq.com">晓超云博客</a>
	</div>
	<div class="sidebar-user-des">
		<span class="sidebar-qq"><a href="tencent://message/?uin=<?php echo $Tconfig["qqhm"]; ?>" rel="nofollow" target="_blank" title=""><i class="fa fa-qq"></i>联系</a></span>
	</div>
</div>
...
function blog_navi(){
	...
	<?php endforeach; ?>
	 <?php if($Tconfig['menu_more']== 1 ){?>
	    <li>
		    <a class="arrow" href="javascript:void(0)">更多功能</a>
			<ul class="pc-level">
				<?php echo $Tconfig['menu_html'];?>
			</ul>
		</li>
	<?php }?>	
		</ul>
...
function blog_comments($comments,$params){
	...
	foreach($user_cache as $k=>$a){
		...
		if($comment['poster']==$name&&$comment['mail']==$mail){
			if($role=="admin"){
				$ls_role='<span class="admin">管理员</span>';
	...
	$comment['poster'] = $comment['poster'] && $comment['poster'] != $isuserlink ? '<a href="/content/templates/meta/inc/go.php/?url='.base64_encode($isuserlink).'" '.$isNofollow.' target="_blank" title="'.$comment['poster'].'">'.$comment['poster'].'</a>'.$ls_role.'' : $comment['poster'];
    ?>
...
function blog_comments_post($logid,$ckname,$ckmail,$ckurl,$verifyCode,$allow_remark){
  ...
  <div id="comment-place">
    <div class="comment-post" id="comment-post">
        <div class="cancel-reply" id="cancel-reply" style="display:none"><div id="respond"><a href="javascript:void(0);" onclick="cancelReply()">取消</a></div></div>
        <a name="respond"></a>
...
function related_logs($logData = array()){   
    ...
    if (!empty($related_logs)) {
        foreach ($related_logs as $val) {$i++;
            $out .= "<div class=\"title\"><span class=\"s-" . $i . "\">" . $i . "</span> <a href=\"" . Url::log($val['gid']) . "\" title=\"" . ($val['title']) . " \">{$val['title']}</a></div>";
        }
    }
