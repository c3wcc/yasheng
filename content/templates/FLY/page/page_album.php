...
$log_content .=	'<style>.box{padding:15px 0 0 25px;float:left}.pic{padding:10px;border:1px solid #ccc;border-radius:5px;box-shadow:0 0 5px #ccc}.pic img{width:185px;height:auto;margin:0!important;}@media screen and (max-width:800px){.box{padding:15px 0 0 5px}}@media screen and (max-width:360px){.box{padding:15px 0 0 55px}}</style>';
if ($album === '') {
    if(empty($kl_album_info)){
			...
			foreach ($kl_album_info as $val){
				...
				$coverPath = str_replace('thum-','',$coverPath); 
...
if ($album !== '') {
    $exist_album = false;
			...
			$log_content .='<script>jQuery(document).ready(function(){var $container = $(".context-album");$container.imagesLoaded(function(){$container.masonry({itemSelector: ".box",gutter: 8,isAnimated: true});});});</script>';
		}else{
			$log_content .= '参数错误。';
		}
}
